...
公开站包含首页、产品目录与详情、羽绒行情、生产工艺、质量检测、文章列表与详情、关于、联系我们、隐私政策和使用条款。`/inquiry/success` 仅作为旧链接兼容提示，不代表网站仍可提交表单。
...
GitHub Actions 每天北京时间 09:50 从中国羽绒工业协会旗下“中国羽绒信息网”读取最新行业条目，并调用受 `AUTH_SECRET` 保护的 `/api/cron/news-sync` 写入文章中心。同步只保存标题、分类、发布日期、简短索引摘要和原文链接，不复制来源正文；来源编号用于去重，同一条资讯再次同步时只会更新。自动资讯最多保留最近 30 天，超期数据会在同步时软删除，网站原有知识文章不受影响。任务复用仓库 Secret `MARKET_SYNC_SECRET`，其值必须与生产环境的 `AUTH_SECRET` 一致。修改抓取规则后可先运行：
