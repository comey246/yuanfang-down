...
品牌名、电话、邮箱和旧站 Logo 已从同一仓库的旧版网站迁入；其他未确认企业信息仍使用“待填写”。演示产品不包含虚构参数、价格、产能、认证或检测结论，演示文章为后台草稿，不会直接公开。
...
旧站出现的百分比规格、产能、合作客户数量、检测次数、出口地区数量和“证书齐全”等表述均未迁移。后台已存在的非占位企业设置不会被迁移脚本覆盖。部署新代码后执行以下命令，才会把上述内容安全合并到当前数据库：
